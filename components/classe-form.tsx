...
interface ClasseFormProps {
  ...
  options: Option[];
  ecoles?: Ecole[]; // Liste des ecoles pour le sélecteur
  role?: string;
  ecoleId?: number;
}
...
export function ClasseForm({
  ...
  const form = useForm<ClasseFormValues>({
    ...
    defaultValues: {
      ...
      idecole: isEcole ? Number(ecoleId) : Number(initialData?.idecole),
    },
  });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              >
                <FormControl>
                  <SelectTrigger className="w-full" disabled={isEcole}>
                ...
                <SelectContent>
                  ...
                  {ecoles?.map((ecole) => (
                    <SelectItem
                      key={ecole.idecole}
                      value={String(ecole.idecole)}
                    >
                      {ecole.nomecole} ({ecole.abreviation})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
