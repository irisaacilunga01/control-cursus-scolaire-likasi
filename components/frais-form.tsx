...
import { Annee_scolaire, Frais_enrolement } from "@/type"; // Importez Frais et Anneescolaire
...
interface FraisFormProps {
  initialData?: Frais_enrolement; // Pour l'édition, contient les données existantes
  anneescolaires: Annee_scolaire[]; // Liste des années scolaires pour le sélecteur
}
    ...
    try {
      ...
      if (initialData) {
        ...
        result = await updateFrais(
          initialData.idfrais,
          dataToSave as Partial<Omit<Frais_enrolement, "idfrais">>
        );
      } else {
        // Mode ajout
        result = await addFrais(
          dataToSave as Omit<Frais_enrolement, "idfrais">
        );
      }
      ...
      if (result.success) {
        ...
        router.push("/dashboard/frais-enrolement"); // Redirige vers la liste des frais après succès
