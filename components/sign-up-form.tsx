...
import { checkEcoleEmailAndGetId } from "@/app/actions/auth"; // Importez la nouvelle fonction d'action
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        ...
        idecole,
      } = await checkEcoleEmailAndGetId(email);

      if (!emailCheckSuccess) {
        toast.error("Inscription impossible", {
          description:
            emailCheckError ||
            "Cet e-mail n'est pas associé à une école enregistrée.",
        });
        setIsLoading(false);
        return;
      }
      ...
      const { error: signUpError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/dashboard`, // Rediriger après confirmation
          data: { role: "ecole", ecole_id: idecole }, // Ajouter le rôle et l'idecole aux métadonnées
        },
      });
      ...
      toast.success("Inscription réussie !", {
        description:
          "Veuillez vérifier votre boite e-mail pour confirmer votre compte.",
      });
