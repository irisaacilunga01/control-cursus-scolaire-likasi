...
import { Annee_scolaire, Classe, Eleve, Inscription, Option } from "@/type"; // Importez tous les types nécessaires
...
export async function getInscriptions(): Promise<{
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("inscriptions")
      .select("*, eleves(*), classes(*, options(*)), annee_scolaires(*)")
...
export async function addInscription(
  ...
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .from("inscriptions") // Nom de la table dans Supabase
...
export async function getRecentInscriptions(limit: number = 5): Promise<{
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("inscriptions")
      .select(
        "*, eleves(nom, prenom, postnom, photo), classes(nom, niveau, options(abreviation)), annee_scolaires(intitule)"
      )
