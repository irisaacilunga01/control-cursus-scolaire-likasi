...
import { Eleve, Frais_enrolement, Paiement } from "@/type"; // Importez tous les types nécessaires
...
export async function getPaiements(): Promise<{
  data?: (Paiement & { eleves?: Eleve; frais_enrolement?: Frais_enrolement })[];
  error?: string;
  success: boolean;
}> {
  try {
    ...
    const { data, error } = await supabase
      .from("paiements")
      .select("*, eleves(*), frais_enrolement(*)")
...
export async function addPaiement(
  ...
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .from("paiements") // Nom de la table dans Supabase
      .insert(newPaiement)
...
export async function getTotalMontantPaiements(): Promise<{
  ...
  try {
    const supabase = await createClient();
    const { data, error } = await supabase.from("paiements").select("montant");
...
//   data?: (Paiement & { eleve: Eleve; frais: Frais_enrolement })[];
//   error?: string;
//   success: boolean;
// }> {
//   try {
...
//       .from("paiements")
//       .select(
//         "*, eleves(nom, prenom, postnom,photo), frais_enrolement(description)"
//       )
...
//     return {
//       data: serializableData as (Paiement & {
//         eleve: Eleve;
//         frais: Frais_enrolement;
//       })[],
//       success: true,
//     };
//   } catch (err) {
