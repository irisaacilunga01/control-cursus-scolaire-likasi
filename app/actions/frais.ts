...
import { Annee_scolaire, Frais_enrolement } from "@/type"; // Assurez-vous d'importer Anneescolaire
...
export async function getFrais(): Promise<{
  data?: (Frais_enrolement & { annee_scolaires?: Annee_scolaire })[];
  error?: string;
  success: boolean;
}> {
  try {
    ...
    const { data, error } = await supabase
      .from("frais_enrolement")
      .select("*, annee_scolaires(*)") // Jointure avec la table 'anneescolaire'
    ...
    return {
      data: data as (Frais_enrolement & { annee_scolaires?: Annee_scolaire })[],
      success: true,
    };
  } catch (err) {
    return {
 ...
 */
export async function getFraisById(idfrais: number): Promise<{
  data?: Frais_enrolement & { annee_scolaires?: Annee_scolaire };
  error?: string;
  success: boolean;
}> {
...
export async function addFrais(
  newFrais: Omit<Frais_enrolement, "idfrais">
): Promise<{ data?: Frais_enrolement; error?: string; success: boolean }> {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .from("frais_enrolement") // Nom de la table dans Supabase
      .insert(newFrais)
    ...
    revalidate();
    return { data: data as Frais_enrolement, success: true };
  } catch (err) {
    return {
...
export async function updateFrais(
  idfrais: number,
  updatedFields: Partial<Omit<Frais_enrolement, "idfrais">>
): Promise<{ data?: Frais_enrolement; error?: string; success: boolean }> {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
