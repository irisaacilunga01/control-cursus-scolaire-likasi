...
import { Classe, Ecole, Option } from "@/type"; // Assurez-vous d'importer Option également
...
export async function getClasses(): Promise<{
  data?: (Classe & { options?: Option; ecoles?: Ecole })[];
  error?: string;
  success: boolean;
}> {
  try {
    ...
    const { data, error } = await supabase
      .from("classes")
      .select("*, options(*),ecoles(*)") // Jointure avec la table 'option'
    ...
    return {
      data: data as (Classe & { options?: Option; ecoles?: Ecole })[],
      success: true,
    };
  } catch (err) {
    return {
...
export async function addClasse(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("classes") // Nom de la table dans Supabase
      .insert(newClasse)
