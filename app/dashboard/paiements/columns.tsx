...
import { Eleve, Frais_enrolement, Paiement } from "@/type"; // Assurez-vous d'importer tous les types
import {
...
type PaiementDisplay = Paiement & {
  eleves?: Eleve;
  frais_enrolement?: Frais_enrolement;
};
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      toast.success("Suppression réussie !!!", {
        description: `Paiement de ${paiement?.eleves?.nom} ${paiement?.eleves?.postnom} ${paiement?.eleves?.prenom} supprimé avec succès !`,
      });
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent className="flex gap-2 justify-center items-center">
            ...
            <p>
              modifier le paiement de ({paiement?.eleves?.nom}{" "}
              {paiement?.eleves?.postnom} {paiement?.eleves?.prenom})
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="flex gap-2 justify-center items-center ">
              ...
              <p>
                supprimer le paiement de ({paiement?.eleves?.nom}{" "}
                {paiement?.eleves?.postnom} {paiement?.eleves?.prenom})
              </p>
            </TooltipContent>
          </Tooltip>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {paiement?.eleves?.nom} {paiement?.eleves?.postnom}{" "}
              {paiement?.eleves?.prenom}) ?
            </DialogDescription>
          </DialogHeader>
...
export const columns: ColumnDef<PaiementDisplay>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const amount = parseFloat(row.original.montant as any); // Cast to any to handle number type from DB
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const frais = row.original.frais_enrolement;
      return frais
        ? `${frais.description} (${frais.montant} Fc)`
        : "Non spécifié";
    },
  },
  {
