...
import { Classe, Ecole, Option } from "@/type"; // Assurez-vous que Classe et Option sont importés
...
type ClasseDisplay = Classe & {
  options?: Option;
  ecoles?: Ecole; // La relation option est incluse pour l'affichage
};
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      toast.success("Suppression réussie !!!", {
        description: `Classe ${classe?.nom} supprimée avec succès !`,
      });
...
export const columns: ColumnDef<ClasseDisplay>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const option = row.original.options;
...

  {
    ...
    cell: ({ row }) => row.original.ecoles?.nomecole,
  },
  {
    accessorKey: "abreviationecole",
    header: ({ column }) => {
      return (
        <Button
          variant="ghost"
    ...
    cell: ({ row }) => (
      <Badge
        variant="outline"
        className={`text-muted-foreground px-1.5 bg-blue-600 text-white
        }`}
      >
        {row.original.ecoles?.abreviation}
      </Badge>
    ),
  },
