...
import { deleteAnneescolaire } from "../../actions/anneescolaire";

// Composant Client pour gérer les actions d'édition et de suppression
function AnneescolaireActionsCell({
  anneescolaire,
}: {
  ...
  const handleDelete = async () => {
    setLoading(true);
    const result = await deleteAnneescolaire(anneescolaire.idannee);
    if (result.success) {
      toast.success("Suppression réussie !!!", {
        description: `Année scolaire ${anneescolaire?.intitule} supprimée avec succès !`,
      });
      ...
      toast.error("Erreur de  suppression !!!", {
        description:
          result.error || "Erreur lors de la suppression de l'année scolaire.",
      });
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              ...
              href={`/dashboard/annee-scolaires/${anneescolaire.idannee}/edit`}
            >
          ...
          <TooltipContent className="flex gap-2 justify-center items-center">
            ...
            <p>modifier l&apos;année scolaire {anneescolaire?.intitule}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
        ...
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="flex gap-2 justify-center items-center ">
              ...
              <p>supprimer l&apos;année scolaire {anneescolaire?.intitule}</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
...
export const columns: ColumnDef<Annee_scolaire>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <AnneescolaireActionsCell anneescolaire={row.original} />
    ),
  },
];
