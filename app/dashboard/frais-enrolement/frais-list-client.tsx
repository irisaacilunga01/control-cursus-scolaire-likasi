...
import { Annee_scolaire, Frais_enrolement } from "@/type"; // Assurez-vous que les types sont corrects
...
type FraisDisplay = Frais_enrolement & {
  anneescolaire?: Annee_scolaire; // La relation anneescolaire est incluse pour l'affichage
};
...
export function FraisListClient({ initialFrais }: FraisListClientProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "frais_enrolement" },
        async (payload) => {
          if (!isMounted) return;

          let fraisData: Frais_enrolement;
          ...
          if (
            ...
            fraisData = payload.new as Frais_enrolement;
            let anneescolaireData: Annee_scolaire | undefined = undefined;
            ...
            // DELETE
            fraisWithRelation = payload.old as Frais_enrolement; // Pour la suppression, on utilise les anciennes données pour le filtrage
          }
